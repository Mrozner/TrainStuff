...
namespace ClaudeSepareted.Services
{
    ...
    public class UnifiedPathfindingService
    {
        ...
        /// <summary>
        /// Find direct route between two platforms using database views
        /// </summary>
        public async Task<List<int>?> FindDirectPlatformRouteAsync(Platforms sourcePlatform, Platforms destinationPlatform)
        {
            ...
            try
            {
                ...
                // Build route using the enhanced pathfinding algorithm
                var route = await BuildPlatformRouteAsync(sourceSectionId, destSectionId, destinationPlatform.DB_ID);
        ...
        /// <summary>
        /// Enhanced route building with better error handling and validation
        /// </summary>
        private async Task<List<int>?> BuildPlatformRouteAsync(int sourceSectionId, int destSectionId, int destinationPlatformId)
        {
            try
            {
                ...
                while (steps < maxSteps)
                {
                    ...
                    var bestConnection = await FindBestConnectionToPlatformAsync(connections, destinationPlatformId, visited);
                ...
                // Fallback: Try simple BFS pathfinding
                var fallbackRoute = await FindBFSToDestinationAsync(sourceSectionId, destSectionId);
        ...
        /// <summary>
        /// Fallback BFS pathfinding to find any route from source to destination section
        /// This is used when the platform-aware pathfinding fails
        /// </summary>
        private async Task<List<int>?> FindBFSToDestinationAsync(int sourceSectionId, int destSectionId)
        {
            try
            {
                ...
                while (queue.Count > 0)
                {
                    ...
                    // Get all connections from current section
                    var connections = await GetSectionConnectionsAsync(currentSection);
        ...
        /// <summary>
        /// Plan complete route with switch configuration
        /// </summary>
            ...
            string trainName = "Unknown")
        {
            ...
            try
            {
                ...
                Console.WriteLine($"[UnifiedPathfinder] Planning route for {trainName}: {sourcePlatformName} -> {destPlatformName}");
                _statusService?.ShowInfo($"Route planning: {sourcePlatformName} → {destPlatformName}", trainName);

                // Find route using platform pathfinding
                Console.WriteLine($"[UnifiedPathfinder] Starting pathfinding from platform {sourcePlatform.DB_ID} (section {sourcePlatform.SubSection_DB_ID}) to platform {destinationPlatform.DB_ID} (section {destinationPlatform.SubSection_DB_ID})");

                var routeSections = await FindDirectPlatformRouteAsync(sourcePlatform, destinationPlatform);
                if (routeSections == null || !routeSections.Any())
                {
                    var errorMsg = $"No route found from {sourcePlatformName} to {destPlatformName}";
        ...
        /// <summary>
        /// Get possible next sections for train pathfinding
        /// </summary>
