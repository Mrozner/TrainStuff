    ...
    public class SwitchConfigurationService
    {
        ...
        public async Task<bool> ConfigureSwitchesForBlockAsync(int currentSectionId, int nextSectionId, string nextSectionName, RoutePlan routePlan, string trainName)
        {
            ...
            try
            {
                ...
                {
                    _logger?.LogWarning("⚠️ No edge found for ({Source} → {Target}) in route plan for {Train}",
                        currentSectionId, nextSectionId, trainName);
                    return true;
                }
                ...
                {
                    _logger?.LogWarning("⚠️ Edge ({Source} → {Target}) has no SwitchRequirements for {Train}",
                        currentSectionId, nextSectionId, trainName);
                    return true;
                }
        ...
        /// <summary>
        /// Synchronizes all physical switches to a safe default state ("straight") sequentially.
        /// CRITICAL: Uses a strict 200ms physical delay between commands to prevent DCC short-circuits.
        /// </summary>
        public async Task<bool> SyncAllSwitchesToDefaultAsync(ApplicationDbContext dbContext)
        {
            try
            {
                _statusService?.ShowInfo("Váltók szinkronizálása indítva...", "SYSTEM");
                _logger?.LogInformation("Starting physical hardware switch synchronization...");

                // FIXED: Query the dedicated Switches table based on the actual DB schema
                var allSwitches = await dbContext.Switches.ToListAsync();

                int successCount = 0;
                string defaultState = "straight";

                // CRITICAL: Strictly sequential loop. DO NOT use Task.WhenAll here.
                foreach (var sw in allSwitches)
                {
                    // FIXED: Use the 'Name' property (e.g., "SW1") as shown in the DB
                    bool success = await SendSwitchCommandAsync(sw.Name, defaultState, "SYSTEM");

                    if (success) successCount++;

                    // CRITICAL: 200ms physical delay to prevent DCC power supply overload
                    await Task.Delay(200);
                }

                _statusService?.ShowSuccess($"Szinkronizáció kész: {successCount}/{allSwitches.Count} váltó beállítva.", "SYSTEM");
                return successCount == allSwitches.Count;
            }
            catch (Exception ex)
            {
                _logger?.LogError(ex, "Error during bulk switch synchronization");
                _statusService?.ShowError($"Szinkronizációs hiba: {ex.Message}", "SYSTEM");
                return false;
            }
        }
