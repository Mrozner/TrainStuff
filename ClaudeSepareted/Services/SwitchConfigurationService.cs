...
namespace ClaudeSepareted.Services
{
    ...
    public class SwitchConfigurationService
    {
        ...
        /// Uses direct memory lookup from RoutePlan instead of database queries
        /// </summary>
        public Task<List<(string SwitchName, string RequiredPosition)>> GetRequiredSwitchesForRouteAsync(RoutePlan routePlan)
        {
            var requiredSwitches = new List<(string SwitchName, string RequiredPosition)>();
            if (routePlan?.Path == null) return Task.FromResult(requiredSwitches);

            foreach (var edge in routePlan.Path)
            {
                if (edge.SwitchRequirements != null)
                {
                    requiredSwitches.AddRange(edge.SwitchRequirements);
                }
            }
            return Task.FromResult(requiredSwitches.Distinct().ToList());
        }

        /// <summary>
        /// Configure all switches required for a route via MQTT
        /// </summary>
        public async Task<SwitchConfigurationResult> ConfigureSwitchesForRouteAsync(RoutePlan routePlan, string trainName)
        {
            try
            {
                var requiredSwitches = await GetRequiredSwitchesForRouteAsync(routePlan);

                // PHASE 3.1: Removed dead code and fallback logic
                // The hardcoded fallback values (switch1, switch2, switch3) were dangerous for production
                // and created dead code immediately afterward with the !requiredSwitches.Any() check
                ...
                var switchesToConfigure = new List<(string SwitchName, string Position)>();

                // Filter out switches that are already in the correct position
                foreach (var (switchName, position) in requiredSwitches)
                {
                    ...
                    switchesToConfigure.Add((switchName, position));
                }

                // Execute remaining switch configurations with a concurrency limit of 3
                if (switchesToConfigure.Any())
                {
                    using var semaphore = new SemaphoreSlim(3);

                    var throttledTasks = switchesToConfigure.Select(async sw =>
                    {
                        await semaphore.WaitAsync();
                        try
                        {
                            return await ExecuteSwitchCommandAsync(sw.SwitchName, sw.Position, trainName);
                        }
                        finally
                        {
                            semaphore.Release();
                    ...
                    // Await all throttled tasks and capture the results directly
                    var results = await Task.WhenAll(throttledTasks);

                    // Process results array safely
                    foreach (var (success, switchName, position, error) in results)
                    {
                        if (success)
                        {
                            configuredSwitches.Add(new SwitchConfiguration(switchName, position));
                        ...
                        else
                        {
                            var errorMsg = error != null
                                ? $"Error configuring switch {switchName}: {error.Message}"
                                : $"Failed to configure switch {switchName} to {position}";
                            errors.Add(errorMsg);
                        }
                    }
                }
        ...
        public async Task<bool> ConfigureSwitchesForBlockAsync(int nextSectionId, string nextSectionName, RoutePlan routePlan, string trainName)
        {
            if (nextSectionId <= 0)
            {
            ...
            try
            {
                // Step 1: Find the edge in the route plan that leads to nextSection
                // PHASE 3.2: Fixed directional routing flaw
                // Changed from blanket OR check to strict TargetNodeId matching for forward progress
                // This prevents matching the wrong edge on bidirectional routes
                // Note: If the domain requires bidirectional travel where trains enter via SourceNodeId,
                // this should be enhanced with direction-aware logic based on train direction
                var edgeForNextSection = routePlan.Path
                    .FirstOrDefault(e => e.TargetNodeId == nextSectionId);
                ...
                // Step 3: Send MQTT commands for all switches concurrently
                var switchTasks = requiredSwitches.Select(async (switchTuple) =>
                {
                    var (switchName, position) = switchTuple;
                    var success = await SendSwitchCommandAsync(switchName, position, trainName);
                    return (Success: success, SwitchName: switchName);
                });
