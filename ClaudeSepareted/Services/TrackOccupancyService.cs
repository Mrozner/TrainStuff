    ...
    public class TrackOccupancyService : IDisposable
    {
        ...
        private async Task HandleSectionOccupiedAsync(string sectionName)
        {
            ...
            try
            {
                ...
                if (trainName == null)
                {
                    // Could not identify train - register as UNKNOWN_OBSTACLE
                    // This prevents pathfinding services from routing other trains through this section
                    trainName = $"UNKNOWN_OBSTACLE_{sectionName}";
                    previousSection = null; // No previous section for obstructions
                    direction = true; // Default direction for obstructions

                    _fileLogger?.Log($"[TRACK OCCUPANCY] ALARM: Unidentified obstruction detected at {sectionName}! Track locked to prevent collisions.");
                    _logger.LogWarning("Unidentified obstruction detected in section {Section}. Registered as {ObstacleID}. Pathfinding will avoid this section.",
                        sectionName, trainName);
                }

                // Update occupancy tracking (works for both trains and obstructions)
            ...
            try
            {
                ...
                lock (_stateLock)
                {
                    ...
                    foreach (var (trainName, state) in _trainStates)
                    {
                        ...
                        // Check if current section is adjacent to new section (synchronous in-memory lookup)
                        var areAdjacent = AreSectionsAdjacent(currentSection, newSection);
