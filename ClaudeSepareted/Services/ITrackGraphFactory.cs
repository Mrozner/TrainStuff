    ...
    public class TrackGraphFactory : ITrackGraphFactory
    {
        ...
        public async Task InvalidateCacheAsync()
        {
            ...
            try
            {
                ...
                _fileLogger?.Log("[TRACK GRAPH] Topology cache invalidated. Rebuilding immediately to prevent latency spikes.");

                // Step 11: Immediately rebuild cache to prevent latency on next request
                // Create a short-lived scope for the database context
                using var scope = _scopeFactory.CreateScope();
                var dbContext = scope.ServiceProvider.GetRequiredService<ApplicationDbContext>();

                _cachedTrackGraph = await TrackGraph.LoadFromDatabaseAsync(dbContext);
                _fileLogger?.Log("[TRACK GRAPH] Topology cache rebuilt successfully.");
            }
            finally
            {
                _semaphore.Release();
