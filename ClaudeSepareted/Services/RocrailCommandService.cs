...
namespace ClaudeSepareted
{
    ...
    public class RocrailCommandService
    {
        ...
        /// <summary>
        /// Synchronizes all physical trains to a safe default state (Speed 0).
        /// Prevents "ghost trains" from starting if Rocrail has lingering MQTT commands from an ungraceful shutdown.
        /// </summary>
        public async Task<bool> SyncAllTrainsToStopAsync(ApplicationDbContext dbContext)
        {
            try
            {
                _statusService?.ShowInfo("Vonatok biztonsági leállítása indítva...", "SYSTEM");
                _logger?.LogInformation("Starting safety stop for all trains on startup...");
                ...
                {
                    // Generate a Speed 0 command
                    var stopCommand = RocrailCommandFactory.TrainVelocity(train.Name, 0, true);

                    // Publish DIRECTLY via _mqttService, bypassing the deduplication caches
                    // This guarantees the hardware is explicitly told to stop, regardless of what C# thinks the state is.
                    bool success = await _mqttService.PublishAsync(_config.RocrailIngressTopic, stopCommand);

                    if (success)
                    {
                        // Update our internal cache to know the train is stopped
                        _lastSpeedCommands[train.Name] = stopCommand;
                        successCount++;
                    }

                    // 100ms delay to prevent network/MQTT flooding
                    await Task.Delay(100);
                }

                _statusService?.ShowSuccess($"Biztonsági leállítás kész: {successCount}/{allTrains.Count} vonat leállítva.", "SYSTEM");
                return successCount == allTrains.Count;
            }
            catch (Exception ex)
            {
                _logger?.LogError(ex, "Error during bulk train stop synchronization");
                _statusService?.ShowError($"Vonat leállítási hiba: {ex.Message}", "SYSTEM");
                return false;
            }
        }
