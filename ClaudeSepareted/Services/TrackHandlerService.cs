    ...
    public class TrackHandlerService : IDisposable
    {
        ...
        private CachedSchedule? GetNextUpcomingEntry(TimeSpan currentTime)
        {
            lock (_lockObject)
            {
                return _cachedTimetableEntries
                    .Where(te => te.StartTime <= currentTime && !_recentlyProcessedTrains.ContainsKey($"entry_{te.Id}"))
                    .OrderBy(te => te.StartTime)
                    .FirstOrDefault();
            }
        }
        ...
        private int CalculateDynamicTimeout(TimeSpan currentTime)
        {
            lock (_lockObject)
            {
                var nextEntry = _cachedTimetableEntries
                    .Where(te => !_recentlyProcessedTrains.ContainsKey($"entry_{te.Id}"))
                    .OrderBy(te => te.StartTime)
                    .FirstOrDefault();
                ...
                // If the next train shares the same start time or is overdue, trigger immediately
        ...
        private async Task RunAsync(CancellationToken cancellationToken)
        {
            ...
            if (await _mqttService.InitializeAsync())
            {
                ...
                // CRITICAL: Wait 1 second after power-on before firing switch solenoids
                // This ensures DCC capacitors are fully charged.
            ...
            // --- HARDWARE SWITCH SYNCHRONIZATION ---
            if (_switchConfigurationService != null)
            {
                using (var scope = _serviceProvider.CreateScope())
                {
                    try
                    {
                        var dbContext = scope.ServiceProvider.GetRequiredService<ApplicationDbContext>();
                        await _switchConfigurationService.SyncAllSwitchesToDefaultAsync(dbContext);
                    }
                    catch (Exception ex)
                    {
                        _logger?.LogError(ex, "Failed to synchronize hardware switches during startup.");
                        _statusService?.ShowError("Figyelmeztetés: Hardver váltók szinkronizálása sikertelen.");
                    }
                }
            }

            // --- TRAIN SAFETY STOP SYNCHRONIZATION ---
            // Prevents "ghost trains" from moving due to lingering MQTT commands from previous sessions
            var rocrailCommandService = _serviceProvider.GetService<RocrailCommandService>();
            if (rocrailCommandService != null)
            {
                using (var scope = _serviceProvider.CreateScope())
                {
                    try
                    {
                        // 1. Reset command history to clear any weird cached states from previous runs
                        rocrailCommandService.ResetCommandHistory();

                        // 2. Safely align all switches
                        // (Already done above in switch synchronization section)

                        // 3. FORCE all trains to Speed 0 to kill any lingering MQTT momentum
                        var dbContext = scope.ServiceProvider.GetRequiredService<ApplicationDbContext>();
                        await rocrailCommandService.SyncAllTrainsToStopAsync(dbContext);
                    }
                    catch (Exception ex)
                    {
                        _logger?.LogError(ex, "Failed to synchronize train safety stops during startup.");
                        _statusService?.ShowError("Figyelmeztetés: Vonatok biztonsági leállítása sikertelen.");
                    }
                }
            }
                    ...
                    try
                    {
                        ...
                        try
                        {
                            ...
                            if (!string.IsNullOrEmpty(sourcePlatform) && !string.IsNullOrEmpty(destPlatform))
                            {
                                ...
                                var routeResult = await _pathfinder.PlanRouteAsync(
                                    timetableEntry.SourcePlatform,
                                    timetableEntry.DestinationPlatform,
                                    train.Name,
                                    timetableEntry.Direction);  // NEW: Use the UI selected direction
