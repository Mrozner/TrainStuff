...
namespace ClaudeSepareted.Services
{
    ...
    public class PathfindingService
    {
        ...
        public async Task<List<int>?> CalculateTrainPathAsync(Train train)
        {
            ...
            var sourceSectionId = _trackGraph.GetSectionIdBySubSectionId(train.SubSection.DB_ID);
            var destSectionId = _trackGraph.GetSectionIdBySubSectionId(train.Destination.SubSection_DB_ID);

            if (sourceSectionId <= 0 || destSectionId <= 0)
                return null;

            // Graph now correctly receives the expected Logical IDs
            ...
            // Convert RoutePlan to List<int> using efficient LINQ
            // REMOVED .Distinct() to allow trains to revisit nodes in complex junctions
            var routeSections = new[] { sourceSectionId }.Concat(routePlan.Path.Select(e => e.TargetNodeId)).ToList();
