...
namespace ClaudeSepareted.Services
{
    ...
    public class PathfindingService
    {
        ...
        public async Task<List<int>?> FindDirectPlatformRouteAsync(Platforms sourcePlatform, Platforms destinationPlatform, bool direction)
        {
            ...
            // REMOVED .Distinct() to allow trains to revisit nodes in complex junctions
            // Fix: Take first source node, then only target nodes to prevent duplication
            var routeSections = new[] { routePlan.Path.First().SourceNodeId }
                .Concat(routePlan.Path.Select(e => e.TargetNodeId))
                .ToList();
        ...
        public async Task<RoutePlan?> FindDirectPlatformRoutePlanAsync(Platforms sourcePlatform, Platforms destinationPlatform, bool direction)
        {
            ...
            // Rely on the native TrackEdge.Direction filtering within FindRouteInMemory
            // to automatically generate distinct physical routes based on train orientation.
        ...
        public async Task<List<int>?> CalculateTrainPathAsync(Train train)
        {
            ...
            // TRANSLATE: Physical Sensor IDs -> Logical Section IDs
            var sourceSubSectionId = new SubSectionId(train.SubSection.DB_ID);
            var destSubSectionId = new SubSectionId(train.Destination.SubSection_DB_ID);

            SectionId? sourceSectionId = _trackGraph.GetSectionIdBySubSectionId(sourceSubSectionId);
            SectionId? destSectionId = _trackGraph.GetSectionIdBySubSectionId(destSubSectionId);

            if (!sourceSectionId.HasValue || !destSectionId.HasValue)
                return null;

            // Graph now correctly receives the expected STRONGLY TYPED Logical IDs
            var routePlan = _trackGraph.FindRouteInMemory(sourceSectionId.Value, destSectionId.Value, train.Direction);
            ...
            // Convert RoutePlan to List<int> using efficient LINQ
            // REMOVED .Distinct() to allow trains to revisit nodes in complex junctions
            // Use .Value.Value: The first gets the struct from the Nullable, the second gets the int from the struct
            var routeSections = new[] { sourceSectionId.Value.Value }.Concat(routePlan.Path.Select(e => e.TargetNodeId)).ToList();
        ...
        /// <summary>
        /// Plan route considering train direction.
        /// Uses pre-calculated routing table for instant lookup (if available).
        /// Does NOT configure switches - pure planning only for JIT operation.
        /// </summary>
            ...
            bool direction = true)
        {
            ...
            try
            {
                ...
                if (cachedRoute != null)
                {
                    ...
                    {
                        _fileLogger?.Log($"[PATHFINDER] Cached route found for {trainName}: {sourcePlatformName} -> {destPlatformName}");
                        _statusService?.ShowSuccess($"Route planned", trainName);
                ...
                _fileLogger?.Log($"[PATHFINDER] Dynamic route generated for {trainName}: {sourcePlatformName} -> {destPlatformName}");
                _statusService?.ShowSuccess($"Route planned", trainName);
