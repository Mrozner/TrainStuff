    ...
    public class TrainMovementMonitor : IDisposable
    {
        ...
        private bool IsBlockBlockedByOtherTrain(string sectionName)
        {
            try
            {
                ...
                {
                    bool isOccupied = _trackOccupancyService.IsSectionOccupied(sectionName);

                    if (isOccupied)
                    {
                        // Get the name of the occupying train
                        if (_trackOccupancyService.TryGetTrainInSection(sectionName, out string occupyingTrain))
                        {
            ...
            try
            {
                ...
                if (currentSpeed != targetSpeed || isStartingFromStop)
                {
                    ...
                    var success = await _rocrailCommandService.SendTrainSpeedCommandAsync(
                        _train.Name,
                        targetSpeed,
                        _train.Direction ? Direction.Forward : Direction.Backward);
        ...
        private async Task ProcessMovementsAsync(ChannelReader<TrainMovedEventArgs> reader, CancellationToken cancellationToken)
        {
            try
            {
                ...
                // CRITICAL: Early exit on cancellation request to prevent task leaks
                // This check complements the one inside the await foreach loop
                if (cancellationToken.IsCancellationRequested)
                {
                    _logger?.LogInformation("TrainMovementMonitor for train {TrainName} received cancellation request at startup - exiting immediately", _train.Name);
                    return;
                }
                    ...
                    try
                    {
                        ...
                        if (!isMyTrain && isWaitingForThisClearance)
                        {
                            ...
                            if (isPhysicallyFree && isLogicallyFree)
                            {
                                ...
                                {
                                    // CRITICAL FIX: We won the lock. Use unified movement authority to configure switches and resume
                                    int currentId = -1;

                                    // Fetch the ID of the block we are currently sitting in safely
                                    lock (_routeIndexLock)
                                    {
                                    ...
                                    if (currentId != -1)
                                    {
                                        ...
                                        // Use unified method with isStartingFromStop = true to ensure proper switch configuration and speed adjustment
                                        await EvaluateMovementAuthorityAsync(currentSectionName, isStartingFromStop: true);
                                    }
                                    else
                                    {
                                        _logger?.LogError("Failed to determine current section for train {TrainName} - [TrainMovementMonitor]", _train.Name);
                                    }
                                }
                                else
                                {
                                    _logger?.LogWarning("⚠️ BLOCK RESERVATION FAILED: {Section} is free but {TrainName} could not reserve it. Continuing to wait.", _waitingForSectionToClear, _train.Name);
                                }
                            }
                            continue;
                        }
                        ...
                        if (!string.IsNullOrEmpty(args.PreviousSubSection) && _blockReservationService != null)
                        {
                            ...
                            // Clear switch configuration cache for the section we just left
                            // This allows switches to be reconfigured if we come back this way later
                            lock (_cacheLock)
                            {
                                _configuredBlocks.Remove(args.PreviousSubSection);
                            }
                        }
                        ...
                        // 3. Evaluate movement authority (unified lookahead logic)
